...
if(isset($_POST['nis'])){
    ...
    // --- 3. CARI DATA SISWA (NIS atau RFID - toleran terhadap perbedaan leading zero) ---
    $input_clean = ltrim($input, '0');
    $stmt_siswa = $conn->prepare("SELECT * FROM siswa WHERE nis = ? OR rfid_uid = ? OR TRIM(LEADING '0' FROM nis) = ? OR TRIM(LEADING '0' FROM rfid_uid) = ?");
    $stmt_siswa->bind_param("ssss", $input, $input, $input_clean, $input_clean);
    $stmt_siswa->execute();
...
function kirim_notifikasi_multi($hp, $tele_id, $email, $pesan, $p, $nis) {
    ...
    // Normalisasi NULL menjadi string kosong untuk mencegah error NOT NULL pada database
    $hp = $hp ?? '';
    $tele_id = $tele_id ?? '';
    $email = $email ?? '';

    // Jika semua saluran komunikasi kosong, tidak perlu mengirim notifikasi apa pun
    if (empty($hp) && empty($tele_id) && empty($email)) {
        return;
    }
