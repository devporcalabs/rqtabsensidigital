...
if(isset($_POST['simpan_pengaturan'])){
    ...
    // PERUBAHAN: Menangani checkbox hari libur (array menjadi string)
    $libur_p = isset($_POST['libur_pekanan']) ? implode(',', $_POST['libur_pekanan']) : '';
...
if(isset($_POST['hapus_data_massal'])){
    ...
    if(!empty($_POST['pilihan_hapus'])){
        foreach($_POST['pilihan_hapus'] as $item){
            if($item == 'absensi') $conn->query("DELETE FROM absensi");
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="d-flex flex-wrap gap-3 mt-1">
                                <?php
                                // Pecah data hari libur dari database menjadi array
                                $hari_libur = isset($data['libur_pekanan']) ? explode(',', $data['libur_pekanan']) : [];
                                $daftar_hari = ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'];
                                
                                foreach ($daftar_hari as $hari) {
                                    // Cek apakah hari ini ada di dalam array hari libur
                                    $checked = in_array($hari, $hari_libur) ? 'checked' : '';
                                    echo "
                                    <div class='form-check'>
                                        <input class='form-check-input' type='checkbox' name='libur_pekanan[]' value='$hari' id='libur_$hari' $checked>
                                        <label class='form-check-label' for='libur_$hari'>$hari</label>
                                    </div>";
                                }
                                ?>
                            </div>
            ...
            <div class="card p-4 border-start border-danger border-4">
                ...
                <form method="POST" onsubmit="return confirm('PERHATIAN! Data akan dihapus selamanya. Apakah Anda sangat yakin?')">
                    ...
                    <div class="form-check mb-2">
                        ...
                        <label class="form-check-label small fw-bold" for="h1">Kosongkan Riwayat Absensi</label>
                    </div>
                    <div class="form-check mb-2">
